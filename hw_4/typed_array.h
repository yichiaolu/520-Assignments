...
public:
    ...
    template <typename T>
    friend TypedArray<T> operator+(const TypedArray<T>& a, const TypedArray<T>& b);
...
template <typename ElementType>
TypedArray<ElementType> operator+(const TypedArray<ElementType>& a, const TypedArray<ElementType>& b) {
    return a.concat(b);
}
